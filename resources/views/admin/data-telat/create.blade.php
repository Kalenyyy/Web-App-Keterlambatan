...
<div class="p-10 mt-10 sm:ml-64">
    ...
    <form class="max-w-md mx-auto mt-10 shadow-lg p-4" method="post" action="{{ route('admin.data-telat.store') }}" enctype="multipart/form-data">
        @csrf
        <h1 class="text-center text-2xl mb-10 font-semibold">Form Input Data Keterlambatan</h1>
        <label for="underline_select" class="sr-only">Underline select</label>
        <select id="underline_select" name="student_id" 
            class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
            @foreach ($students as $student)
                <option selected hidden disabled>Choose Student</option>
        ...
        <div class="relative z-0 w-full mb-5 group mt-5">
            <input type="datetime-local" name="date_time_late" id="date_time_late" 
                ...
                placeholder=" " required value="{{ $today }}"/>
